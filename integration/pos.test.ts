...
describe('POS', () => {
  ...
  describe('EVM', () => {
    it('should build an ERC20 transfer transaction', async () => {
      ...
      const params = result.transactionRpc.params[0];
    ...
    it('should not build a transaction with a recipient that is not a valid address', async () => {
      ...
      expect(response.data.error.message.includes('Invalid Recipient')).toBe(true);
    });
    ...
    it('should not build a transaction with an invalid asset', async () => {
      ...
      expect(response.status).toBe(400);
      expect(response.data.error.message.includes('Validation error')).toBe(true);
    });
