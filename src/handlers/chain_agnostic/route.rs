use {
    super::{
        ...
        get_assets_changes_from_simulation, is_supported_bridging_asset, BridgingStatus,
    ...
    let (asset_transfer_contract, asset_transfer_value) =
        match decode_erc20_transfer_data(&transaction_data) {
            Ok((_, erc20_transfer_value)) => {
                ...
                (to_address, erc20_transfer_value)
            }
            _ => {
                debug!(
                ...
                (asset_transfer_contract, asset_transfer_value)
            }
        };
