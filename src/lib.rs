...
fn init_providers(config: &Config) -> ProviderRepository {
    ...
    providers.add_provider(Arc::new(infura_provider), 1);

    let infura_ws_provider = InfuraWsProvider {
        project_id: infura_project_id,
        supported_chains: infura_ws_supported_chains,
    };
    providers.add_ws_provider("infura".into(), Arc::new(infura_ws_provider), 1);
    ...
    providers.add_provider(Arc::new(pokt_provider), 1);
    ...
    providers.add_provider(Arc::new(binance_provider), 1);
    ...
    providers.add_provider(Arc::new(zksync_provider), 1);
